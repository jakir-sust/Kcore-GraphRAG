...
async def run_workflow(
    ...
    community_algo = config.cluster_graph.community_algo
...
def create_communities(
    ...
    cluster_type = community_algo
    print("config: ", cluster_type)

    # following is for leiden
    ...
    elif cluster_type in {"RkH","M2hC", "MRC"}:
        clusters = kcore_cluster_graph(
            graph,
            use_lcc, cluster_type
        )
    ...
    print("Comunity len = ", len(communities))
    ...
    for level in range(max_level + 1):
        ...
        # Keep all communities after merge with left join (keep communities even with single node, actual code doesn't handle it)
        grouped = communities_at_level.merge(
            grouped,
            left_on=["community", "level", "parent"],
            right_on=["community_x", "level_x", "parent_x"],
            how="left"
        )

        # Remove duplicate columns (avoid InvalidIndexError)
        grouped = grouped.loc[:, ~grouped.columns.duplicated()]
        grouped = grouped.groupby(["community", "level", "parent"], as_index=False).agg({
          "relationship_ids": lambda x: [i for sublist in x if isinstance(sublist, list) for i in sublist],
          "text_unit_ids": lambda x: [i for sublist in x if isinstance(sublist, list) for i in sublist]
        })

        # Fill NaN relationship/text_unit lists safely (fill missing data with empty lists instead of NaN)
        grouped["relationship_ids"] = grouped["relationship_ids"].apply(lambda x: x if isinstance(x, list) else [])
        grouped["text_unit_ids"] = grouped["text_unit_ids"].apply(lambda x: x if isinstance(x, list) else [])
    ...
    # print("Final communities len = ", len(final_communities))
